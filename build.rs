...
fn main() {
    ...
    // `build.rs` is compiled for the host, so `cfg!(target_os = "macos")`
    // describes the developer machine rather than the Rust target. During a
    // macOS -> Android cross-build that incorrectly requested ggml-blas even
    // though Android's CMake configuration never built it.
